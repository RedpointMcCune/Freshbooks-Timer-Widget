...
// Adapted for Windows by Andrew Flusche.
...
ns.Request = function(method, opts) {
	...
	if (ns.debug) System.Debug.outputString(r);
	return r;
}
...
ns.GetXMLHttpRequest = function(name, token, async) {
	...
	if (ns.debug) System.Debug.outputString(target);
	return r;
}
...
ns.fetchFullList = function (
		...
		callback,
		state) {
	if (ns.debug) System.Debug.outputString("fetchFullList:"+ns.print_r([name,token,type,args,timeout,callback,state]));
