...
com.freshbooks.api.UA = "FreshBooks Vista Sidebar Gadget";
...
function load()
{
	...
	checkUpdate();
	//setElementVisibility("update", false);

	dockStateChanged();

	// Check if sitename and token exist.
	if (!validateSitename(sitename) || !validateToken(token)) {

		setElementVisibility("gadgetContent2", false);
		setElementVisibility("gadgetContent1", true);
	
	} else {
	
		setElementVisibility("gadgetContent2", true);
		setElementVisibility("gadgetContent1", false);
		
		setElementVisibility("flyoutHide", false);

		$("#url").text(sitename + ".freshbooks.com");
		$("#submitStatus").text("");
		$("#statusmsg").text("");
...
function editClockedTime(event)
{
	...
	//$("#submithours")[0].disabled = true;
	var curtime = $("#count")[0].innerText;
...
function setTasksFromProject() {
	...
	for (var i in sortedList)
	{
		...
		//tasks.add(o);
		tasks.options[i] = new Option(truncate(sortedList[i].name), sortedList[i].id);
	}
}
...
function loadTasks()
{
	...
	if (!validateSitename(sitename)) { setStatus("statusmsg", "Invalid site name"); return false; }
	...
	setStatus("statusmsg", "Loading " + taskCount + " more...", true, xmlTimeout);

	for (pid in myProjects)
	{
		com.freshbooks.api.fetchFullList(sitename,token,'task',{project_id:pid},xmlTimeout,
...
function fillOutProjects()
{
	...
	for (var i in sortedList)
	{
		...
		// We have to set innerHTML, otherwise things like &amp; will show up literally.
		pr.options[i] = new Option(truncate(sortedList[i].name), sortedList[i].id);
		//o.innerHTML = sortedList[i].name.replace(/</g, "&lt;");
		//pr.add(o);
	}
}
...
function loadProjects()
{
	...
	com.freshbooks.api.fetchFullList(sitename,token,'project',{},xmlTimeout,
		function (list) {
			if (list.status.ok) {
...
function submitHours()
{
	...
	var r = com.freshbooks.api.GetXMLHttpRequest(sitename, token, true);
	...
	$("#submithours")[0].disabled = true;
	setElementVisibility("submithours",  false);

	function enableInputs() {
		$("#Projects")[0].disabled    = false;
		$("#Tasks")[0].disabled       = false;
		$("#Notes")[0].disabled       = false;
		$("#submithours")[0].disabled = false;
		setTimeout('setElementVisibility("submithours",  true)', 3500);
	};
...
// Searches contacts for given string.
//
...
function gotofreshbooks() {
	window.open("https://" + sitename + ".freshbooks.com");
}
...
function checkUpdate() {
	...
	var myurl = "http://www.andrewflusche.com/freshbooksversion.xml";
    myRand = parseInt(Math.random()*999999999999999);
    var modurl = myurl+"?rand="+myRand;
    http.open("GET", modurl, true);
	http.onreadystatechange = compareVersions;
	http.send(null);
	...
	function compareVersions() {
		if (http.readyState == 4) {
			if(http.status == 200) {
				...
				var serverVersion = http.responseXML.getElementsByTagName("version")[0].text;
				
				// Compare to current version.
				if (System.Gadget.version != serverVersion) {
					setElementVisibility("update", true);
				}
			}
		}
	}
